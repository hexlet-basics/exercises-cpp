...
Текст `Hello, World!` появится на экране благодаря команде `std::cout <<`. Такая команда выводит на экран информацию, которая указана после оператора << `'Hello, World!'`. Оператор `<<` позволяет строить цепочки, например: `std::cout << "Hello, World!" << "\n";` То есть теперь информация после _Hello, World!_ будет выводиться на экране на следующей строке.
